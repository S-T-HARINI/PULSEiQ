...
class ScenarioWhatIfRequest(BaseModel):
    scenario_type: ScenarioType = Field(
        ...,
        description="Scenario type: extreme_heatwave, solar_ramp_down, n1_line_trip, wind_storm",
...
class ScenarioWhatIfResponse(BaseModel):
    ...
    generation_mw: float = Field(..., description="Estimated total generation under scenario in MW")
    demand_mw: float = Field(..., description="Estimated total demand under scenario in MW")
    renewable_share_percent: float = Field(..., description="Estimated renewable generation percentage (0-100%)")
    risk_index: float = Field(..., description="Calculated operational risk index (0.0 to 1.0)")
