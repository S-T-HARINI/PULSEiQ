  ...
  return (
    <div className="min-h-screen bg-[#07090e] text-slate-100 flex flex-col selection:bg-amber-500 selection:text-slate-950">
      ...
      <main className="flex-1 max-w-[1600px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 bg-tech-radial">
        ...
        {apiError && (
          <div className="rounded-2xl border border-amber-500/30 bg-amber-950/30 p-4 text-xs font-mono flex items-center justify-between gap-3 text-amber-300 shadow-[0_0_20px_rgba(245,158,11,0.15)] backdrop-blur-xl">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-200 font-bold flex items-center gap-1.5 cursor-pointer transition-all shrink-0"
            >
      ...
      {simulationModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-xl">
          <div className="relative w-full max-w-xl bg-[#090d16]/95 border border-amber-500/40 rounded-2xl p-6 shadow-[0_25px_60px_rgba(0,0,0,0.8)] space-y-5">
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-3.5">
              <div className="flex items-center gap-2.5 text-amber-400 font-mono font-bold text-sm tracking-wide">
                <Terminal className="w-4 h-4" />
                <span>N-1 CONTINGENCY SIMULATION ENGINE</span>
              </div>
              <button
                onClick={() => setSimulationModalOpen(false)}
                className="text-slate-400 hover:text-white font-mono text-sm cursor-pointer p-1 rounded-lg hover:bg-slate-850"
              >
                ✕
              </button>
            </div>
            ...
            {simulating ? (
              ...
              <div className="space-y-4">
                <div className="p-4 bg-slate-950/80 rounded-xl border border-slate-800/80 font-mono text-xs space-y-2 text-slate-300 shadow-inner">
                  ...
                  <div className="flex justify-between text-slate-500 pt-2 border-t border-slate-900">
                ...
                {simResult.simulation_warnings?.length > 0 && (
                  <div className="p-3 rounded-xl bg-amber-950/40 border border-amber-500/30 text-[11px] font-mono text-amber-300 shadow-xs">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button
                onClick={() => setSimulationModalOpen(false)}
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-mono font-bold text-xs shadow-lg shadow-amber-500/20 cursor-pointer transition-all"
              >
      ...
      {scenarioModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-xl">
          <div className="relative w-full max-w-xl bg-[#090d16]/95 border border-cyan-500/40 rounded-2xl p-6 shadow-[0_25px_60px_rgba(0,0,0,0.8)] space-y-5">
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-3.5">
              <div className="flex items-center gap-2.5 text-cyan-400 font-mono font-bold text-sm tracking-wide">
            ...
            {scenarioLoading ? (
              ...
              <div className="space-y-4 font-mono text-xs text-slate-300">
                <div className="p-4 bg-slate-950/80 rounded-xl border border-slate-800/80 space-y-2 shadow-inner">
                  <div className="flex justify-between">
                ...
                {scenarioResult.recommended_response?.length > 0 && (
                  <div className="p-3.5 rounded-xl bg-slate-950/80 border border-slate-800/80 space-y-1.5 shadow-inner">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button
                onClick={() => setScenarioModalOpen(false)}
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-cyan-600 hover:from-cyan-400 hover:to-cyan-500 text-slate-950 font-mono font-bold text-xs shadow-lg shadow-cyan-500/20 cursor-pointer transition-all"
              >
      ...
      {optModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-xl">
          <div className="relative w-full max-w-xl bg-[#090d16]/95 border border-blue-500/40 rounded-2xl p-6 shadow-[0_25px_60px_rgba(0,0,0,0.8)] space-y-5">
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-3.5">
              <div className="flex items-center gap-2.5 text-blue-400 font-mono font-bold text-sm tracking-wide">
              ...
              <div className="space-y-4 font-mono text-xs text-slate-300">
                ...
                <div className="p-3.5 rounded-xl bg-slate-950/80 border border-slate-800/80 space-y-1.5 shadow-inner">
                  ...
                  {optResult.recommended_actions.map((rec, idx) => (
                    <div key={idx} className="flex items-start gap-2 text-blue-300 text-[11px]">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button
                onClick={() => setOptModalOpen(false)}
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-slate-950 font-mono font-bold text-xs shadow-lg shadow-blue-500/20 cursor-pointer transition-all"
              >
      ...
      {/* Footer */}
      <footer className="border-t border-slate-800/80 bg-[#07090e]/90 backdrop-blur-xl py-5 mt-auto">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between text-xs font-mono text-slate-400 gap-3">
          <div className="flex items-center gap-2.5">
            <span className="font-bold text-slate-200 tracking-wider">PULSEiQ</span>
            <span className="text-slate-600">|</span>
            <span className="text-slate-400">NEXT-GENERATION ELECTRICITY GRID DIGITAL TWIN</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1.5">
              <span className={`w-2 h-2 rounded-full ${apiError ? "bg-amber-400" : "bg-emerald-400 animate-pulse"}`}></span>
              API: {apiError ? "FALLBACK" : "CONNECTED"}
            </span>
